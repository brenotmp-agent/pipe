...
Status: em andamento
Owner: engenharia
Last updated: 2026-07-22 (revisado)
...
- [ ] `.gitignore` não bloqueia versionamento dos diretórios de estado (apenas os exclui do repositório da esteira — ver §9)
- [ ] `docker compose config` valida sem erro com e sem `.env`
- [ ] `Dockerfile` declara `WORKDIR /app`

---

## §9 — `.gitignore` e os diretórios de estado

O `.gitignore` do repositório **já contém** as entradas `.pipe/`, `logs/`, `repo/`
e `contexts/`. Isso é **correto e intencional**: esses diretórios são criados em
runtime no host do operador e não pertencem ao repositório da esteira.

A verificação de D-05 (ponto 4 do "O que fazer") não exige remover essas entradas.
O objetivo é confirmar que as regras existentes não impedem o operador de versionar
o estado **em outro repositório** se desejar — e de fato não impedem, pois o
`.gitignore` da esteira se aplica apenas ao repositório da esteira em si.

**Conclusão:** nenhuma alteração no `.gitignore` é necessária para D-05. O
checklist marca este item como satisfeito pela análise: as entradas existentes
excluem corretamente os diretórios do repositório da esteira sem proibir o
operador de gerenciá-los externamente à vontade.
